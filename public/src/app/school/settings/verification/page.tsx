  ...
  useEffect(() => {
    const fetchKycStatus = async () => {
      try {
        ...
        console.error('Error fetching School verification status:', error);
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-8 mb-8 border border-blue-100">
        ...
        <div className="flex items-center justify-between mb-5">
          <p className="text-sm text-gray-700">
            Complete your school verification to unlock these features:
          </p>
          <button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="flex items-start space-x-3">
            ...
            <p className="text-sm text-gray-800">Pre-approve student KYC submissions</p>
          </div>
          <div className="flex items-start space-x-3">
            <span className="text-lg">✓</span>
            <p className="text-sm text-gray-800">Receive and confirm scholarship payments</p>
          </div>
          <div className="flex items-start space-x-3">
            <span className="text-lg">✓</span>
            <p className="text-sm text-gray-800">Select scholars for auto-mode scholarships</p>
          </div>
          <div className="flex items-start space-x-3">
            <span className="text-lg">✓</span>
            <p className="text-sm text-gray-800">Generate reports and payment confirmations</p>
          </div>
        </div>
      </div>

      {kycStatus === 'denied' && statusDetails?.denialReason && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
          <p className="text-sm text-red-600">
            Your verification was denied. Reason: {statusDetails.denialReason}
          </p>
        </div>
      )}
